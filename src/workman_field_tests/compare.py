...
def load_result(path: Path) -> dict[str, Any]:
    ...
    if data.get("schema_version") != "workman-field-tests.v1":
...
def markdown_table(results: list[dict[str, Any]]) -> str:
    ranked = sorted(
        ...
        key=lambda row: (
            ...
            -(row.get("summary", {}).get("median_generation_tokens_per_second") or 0),
        ),
    )
    lines = [
        "| Rank | Model | Hardware | Runtime | Quality | Median gen tok/s |",
        "|---:|---|---|---|---:|---:|",
    ]
    for index, row in enumerate(ranked, 1):
        summary = row["summary"]
        speed = summary.get("median_generation_tokens_per_second")
        lines.append(
